...
@csrf_exempt
def get_comments_beLongOf(request, user_id):
    data = []
    resp = {}
    comment_data = list(comment_Model.objects.filter(userId=user_id))
    # data.append(serialize("json", comment_data, fields=(
    #     "content", "userId", 'userName', 'dateComment')))
    data = [model_to_dict(thing) for thing in comment_data]
    # for datas in data:
    #     data.append()
